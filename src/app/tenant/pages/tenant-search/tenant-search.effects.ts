...
import { CreateSearchConfigRequest, SearchConfigBffService, TenantBffService } from '../../../shared/generated'
...
export class TenantSearchEffects {
    ...
    return this.actions$.pipe(
      ...
      switchMap(([dialogResult, viewState]) => {
        ...
        const request: CreateSearchConfigRequest = {
          ...
          columns: dialogResult.result?.saveColumns ? viewState.columns.map((c) => c.id) : [],
          values: dialogResult.result?.saveInputValues
            ? Object.fromEntries(
                Object.keys(viewState?.searchCriteria ?? {}).map((k) => [
                  k,
                  (viewState?.searchCriteria as Record<string, any>)[k].toString()
                ])
              )
            : {}
        }
  ...
  updateSearchConfig$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      concatLatestFrom(() => this.store.select(tenantSearchSelectors.selectSelectedSearchConfig)),
